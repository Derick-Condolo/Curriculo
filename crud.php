...
try {
    ...
    function readata($pdo, $table, $table2, $foreignKey, $primaryKey, $where = null) {
        $sql = "SELECT $table.*, $table2.* FROM $table ";
        $sql .= "INNER JOIN $table2 ON $table.$primaryKey = $table2.$foreignKey";
        if ($where) {
            $sql .= " WHERE $where";
        }
        $stmt = $pdo->query($sql);
    ...
    function readFk($pdo, $table, $table2, $foreignKey, $primaryKey, $where = null) {
        $sql = "SELECT $table2.* FROM $table ";
        $sql .= "INNER JOIN $table2 ON $table.$primaryKey = $table2.$foreignKey";
        if ($where) {
            $sql .= " WHERE $where";
        }
